...
type BuyCryptoFormValues = z.infer<typeof formSchema>;
...
const BuyCrypto = ({ onComplete, initialCrypto }: BuyCryptoProps) => {
  const { t } = useContext(LanguageContext);
  const [location] = useLocation();
  const { marketData, wallets } = useMarketData();
  const [toAmount, setToAmount] = useState("0");

  const form = useForm<BuyCryptoFormValues>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      fromCurrency: "USDT",
      toCurrency: initialCrypto || "BTC",
      fromAmount: "100",
    },
  });
  ...
  const onSubmit = (values: BuyCryptoFormValues) => {
