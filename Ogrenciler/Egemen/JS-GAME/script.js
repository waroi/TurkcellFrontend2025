...
alert('Lütfen karakter seçiniz! 1: Warrior , 2: Mage, 3: Elf')
...
console.clear()
console.log(
  'Karanlık Diyar’ın sınırlarında, terk edilmiş bir kamp alanına rastladı. Eskiden burada konaklamış bir gezginin geride bıraktığı, toza bulanmış Eski Yolcunun Pelerini hâlâ sapasağlamdı. Kumaşı tuhaf bir şekilde hafifti, ama geceye karıştığında gözle görülmez hale geliyordu.'
)
alert('Devam et')
console.clear()
...
console.log(
  'Diyarın derinliklerine indikçe, rüzgârın fısıltıları yerini hayaletlerin sessizliğine bıraktı. Eski bir savaş alanının ortasında, kimin olduğu bilinmeyen iskeletlerin arasında parlayan bir kılıç gördü. Gölgeli Kılıç adı verilen bu silah, saf bir karanlıktan dövülmüş gibi duruyordu.'
)
alert('Devam et')
console.clear()
item_2 = {
...
Player.type.physical_damage =
  Player.type.physical_damage + item_2.positive_damage
console.log(Player)
alert('Devam et')
console.clear()
...
console.log(
  'Sonunda, Karanlık Diyar’ın merkezine ulaştı. Burada, unutulmuş bir tapınağın harabeleri yükseliyordu. Zamanın bile unutmaya yüz tuttuğu bu yapının içinde, ağır mühürlerle kilitlenmiş Rünlü Zırh saklıydı.'
)
alert('Devam et')
console.clear()
...
for (let i = 0; i < 3; i++) {
  ...
  boss_list = [
    ...
    { name: 'gölgeYaratıgı', hit: 50, health: 150 },
  ...
  if (Player.health > 0) {
    ...
    if (Player.health <= 50) {
      console.log(
        'Kutsamaci saman belirir ve her turu gectiginde eger cani 50 altina düsmüsse onu kutsar ve canini 100 artirir'
      )
      alert('Devam et')
      console.clear()
